...
{
	/// <summary>Maps CreatorProfile to CreatorProfileReadDto. Discovered via IObjectMap scan.</summary>
	public class CreatorProfileToCreatorProfileDtoMap : ObjectMapBase<CreatorProfile, CreatorProfileReadDto>
	{
		/// <inheritdoc/>
		protected override void ConfigureMapping()
		{
			this.CreateMap().MapFrom(dest => dest.Id, src => src.Id).MapFrom(dest => dest.PersonId, src => src.PersonId).MapFrom(dest => dest.Title, src => src.Title).MapFrom(dest => dest.Description, src => src.Description).MapFrom(dest => dest.EraFrom, src => src.EraFrom).MapFrom(dest => dest.EraTo, src => src.EraTo).MapFrom(dest => dest.CreativeMediumId, src => src.CreativeMediumId).MapFrom(dest => dest.Nationality, src => src.Nationality);
		}
	}
}
